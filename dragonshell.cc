...
void commandHandler(string commandstr){
...
int main(int argc, char **argv) {
  ...
  while(1){
    ...
    for (unsigned int i =0; i< commands_str.size(); ++i) {
      ...
      size_t find_redirecting = commands_str[i].find(">");
      size_t find_background = commands_str[i].find("&");
      if(find_redirecting > 2 && find_redirecting != string::npos){
        int saved_stdout;
        saved_stdout = dup(1);
        ...
        commandHandler(redirected_command);
        dup2(saved_stdout, 1);
        close(saved_stdout);
      }

      // for background process
      else if(find_background > 2 && find_background != string::npos){
        int saved_stdout;
        saved_stdout = dup(1);
        int pos = commands_str[i].find("&");
        string file_name = "/dev/null";
        int file_desc = open(file_name.c_str(), O_CREAT | O_WRONLY, 0666);
        if(file_desc < 0){
          cout << "Error opening the file "<< endl;
        }
        dup2(file_desc, 1);
        ...
        string redirected_command = commands_str[i].erase(pos-1);
        
        commandHandler(redirected_command);
        dup2(saved_stdout, 1);
        dup2(saved_stdout, 2);
        //cout << redirected_command << endl;
        close(saved_stdout);
